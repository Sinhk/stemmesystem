...
public interface ISakService
{
    ...
    Task<LagreResult<SakDto>> LagreNySak(SakInputModel input, CancellationToken cancellationToken = default);
    Task<SakDto> OppdaterSak(SakInputModel input, CancellationToken cancellationToken = default);
    ...
    Task<LagreResult<VoteringDto>> LagreNyVotering(VoteringInputModel input, CancellationToken cancellationToken = default);
    Task<LagreResult<VoteringDto>> OppdaterVotering(VoteringInputModel input, CancellationToken cancellationToken = default);
    //Task<ICollection<SakDto>> HentSaker(int arrangementId);
    Task<ICollection<AdminSakDto>> HentSaker(SakerRequest request, CancellationToken cancellationToken = default);
...
public static class LagreResult
{
    ...
    public static LagreResult<T> Error<T>(string error) => new(default, new Dictionary<string, List<string>> { [""] = [error] });
}
