...
public class StemmeService : IStemmeService, IAdminStemmeService
{
    ...
    public async Task<List<StemmeDto>> AvgiStemmeAsync(AvgiStemmeRequest request, CallContext context = default)
    {
        ...
        var dto = stemmer.Select(s => s.ToDto()).ToList();
        return dto;
    }
    ...
    public async Task<HentResult<AdminVoteringDto>> StartVotering(AdminStemmeRequest request, CancellationToken cancellationToken = default)
    {
        ...
        await _context.SaveChangesAsync(cancellationToken);
        var dto = votering.ToAdminDto();
        var e = new VoteringStartetEvent(dto);
        ...
        return new HentResult<AdminVoteringDto>(dto);
    }
